...
TMultiGraph* pod_stage_plots(Data* datum, StageData* stageDatum, Int_t& maxSample, Int_t& minSample)
{
    ...
    data.push_back(datum->zData);//Black
    data.push_back(stageDatum->pmtData);//Red
    data.push_back(stageDatum->pmtCableData);//Green
    data.push_back(stageDatum->ampData);//Blue
    data.push_back(stageDatum->feedthroughCableData);//Yellow
    data.push_back(stageDatum->trigData);//Purpule
    for(int i = 0; i<2; ++i){
